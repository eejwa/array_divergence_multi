...
fig_r = plt.figure(figsize=(15,5))
fig_s = plt.figure(figsize=(15,5))


for f,freq in enumerate([[0.1, 0.2], [0.15, 0.3], [0.2, 0.4]]):
    fmin = freq[0]
    fmax = freq[1]
    f_col = 130 + f + 1
    ax_r = fig_r.add_subplot(f_col)
    ax_r.hist(depths_hists_r_freqs[f], bins=depths, histtype='bar', color='grey', edgecolor='white', align='mid')
    ax_r.set_xlabel(f"Depths (km)")
    ax_r.set_ylabel(f"Number of values in bin")
    ax_r.set_title(f"{region} Receiver-Side: {fmin:.2f} $-$ {fmax:.2f} Hz")
# plt.tight_layout()
plt.savefig(f"histogram_variance_all_freq_{region}_r_side.pdf")
plt.show()
...
for f,freq in enumerate([[0.1, 0.2], [0.15, 0.3], [0.2, 0.4]]):
    ...
    ax_s = fig_s.add_subplot(f_col)
    ax_s.hist(depths_hists_s_freqs[f], bins=depths, histtype='bar', color='grey', edgecolor='white', align='mid')
    ax_s.set_xlabel(f"Depths (km)")
    ax_s.set_ylabel(f"Number of values in bin")
    ax_s.set_title(f"{region} Source-Side: {fmin:.2f} $-$ {fmax:.2f} Hz")
# plt.tight_layout()
plt.savefig(f"histogram_variance_all_freq_{region}_s_side.pdf")
plt.show()
