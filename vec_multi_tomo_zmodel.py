...
tomo='/Users/jamieward/PhD/Models/LLNL_3D_Ray_Tracing/plume_europe/ZMODEL_EU60/ZMODEL_M60.dat'
...
depths_tomo = np.array(list(set(sst[:,2])))


tomo_lower_mantle = '/Users/jamieward/PhD/Figures/Tomography_Models/GYPSUM_percent.nc'
sst_lower_mantle = nc.Dataset(tomo_lower_mantle)
lon_lower_mantle = sst_lower_mantle.variables['longitude'][:].data
lat_lower_mantle = sst_lower_mantle.variables['latitude'][:].data
dvs_lower_mantle = sst_lower_mantle.variables['dvs'][:].data
dvs_new_lower_mantle = np.where(dvs_lower_mantle > 100, 0, dvs_lower_mantle)
dvs_new_lower_mantle = np.where(dvs_new_lower_mantle < -100, 0, dvs_new_lower_mantle)
depths_tomo_lower_mantle = sst_lower_mantle.variables['depth'][:].data

tomog_interp_vs_lower_mantle = interp1d(depths_tomo_lower_mantle, dvs_new_lower_mantle, axis=0, bounds_error=False, fill_value='extrapolate')
...
for d,depth in enumerate(depths):
    ...
    # find nearest depth in depths_tomo to depth 
    idx = (np.abs(depths_tomo - depth)).argmin()
    nearest_depth = depths_tomo[idx]
    print(depth)
    print(nearest_depth)
    ...
    if depth <= 1000:
        tomo_model='EU60'
        dvs_depth = dvs[np.where(sst[:,2] == nearest_depth)[0]] * 100 # convert to %
        lons = lon[np.where(sst[:,2] == nearest_depth)[0]]
        lats = lat[np.where(sst[:,2] == nearest_depth)[0]]
        ...
        # dvs_depth = tomog_interp_vs_lower_mantle(depth)
    ...
    if depth <= 1000:
        m = ax3.tricontourf(lons, lats, dvs_depth, levels=20,
